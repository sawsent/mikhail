...
def list_words(directory: str):
    ...
    words = dict(sorted({word: len(occurences) for word, occurences in transcript.words.items()}.items(), key=lambda kv: (kv[1], kv[0]), reverse=True))

    print(f"Found {len(words)} words in current directory.")
